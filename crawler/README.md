...
`noobdream-import` 默认把题面的**公开样例**提取出来写进 ZIP 的 `tests/` 目录，
导入后每个样例都会成为题目的一个测试点，题目立刻可判，而不是"导入了但没法提交"。

* 测试点带 `sample=true` 标记，前端会当作公开样例展示；
* 样例块含多组用例时**按行拆成多个测试点**（见下）；
* 题目没有样例（源站 `pre#input` / `pre#output` 为空）时不生成测试点，仍需人工补数据；
* 用 `--no-sample-test` 可以退回"只写题面、不含测试点"的旧行为。

**分值怎么给**：导入契约要求测试点分值之和正好为 100，所以转换器按测试点数量均分，
余数补给前几组。整块单点 → 100 分；拆成 2 组 → 50+50；4 组 → 25×4；3 组 → 34+33+33。

**多组样例会被自动拆分**：源站把多组样例拼在同一个 `<pre>` 里。题目 1002 的样例输入是
`2 100` / `2 22` 两行、输出是 `20` / `6` 两行，实际是两组独立用例——整块当成一个测试点，
只处理单组的正确程序会判 WA。转换器按「输入输出行数相同且都大于 1」识别这种写法，
拆成多个测试点并在命令结尾点名：

```
已把 17/20 道题的公开样例写成测试点（共 21 个，标记为公开样例）。
...
```
noobdream:1002  #1 score= 50 | in='2 100' -> out='20'
                #2 score= 50 | in='2 22'  -> out='6'
noobdream:1091  #1 score= 25 | in='850'  -> out='discount=1,pay=850'
...
判据是启发式的（单组用例即使输入输出各有多行，行数通常也不相等，例如"第一行 n、第二行数组"
对应"一行答案"）。实测 20 道真题里命中 1091、1002 两道且都拆对了，其余没有误报。
拆出来的测试点仍然带公开样例标记，建议在管理页面扫一眼确认。

> **只从样例取数据**。转换器不会凭空生成期望输出——那需要可信的标程，否则产出的测试数据
> 会让正确程序判 WA。需要更多测试点请走项目的 AI 测试点生成流程，导出包里的 `tests/`
> 结构（`cases.csv` + `N.in` / `N.out`）与它是兼容的。
