...
SYSTEM_TEST_DATA = (
	...
	"① 输入生成器（generator_source）：只把 argv[1] 当作十进制种子（沙箱调用时 argv[1] 一定存在），"
	"据此向 stdout 打印一份完整测试输入。它不求解题目、不打印答案、不读取 stdin、不依赖时间或环境。\n"
	"② 输入校验器（validator_source）：只从 stdin 读入一份输入，按题面约束检查规模、值域与格式，"
	"用退出码表达结论（0=合法，非 0=非法），不向 stdout 打印答案。\n"
	"把解法写进生成器、或让生成器输出答案，都算错误输出。\n"
	...
	"提交前自检：生成器里出现 argv 且通篇只打印输入；校验器只读 stdin 且只用退出码表达结论；两份源码都不含链接、伪代码或省略号。"
)
