...
导入后它就成为题目的一个测试点，题目立刻可判，而不是"导入了但没法提交"。

```
tests/problem-1/cases.csv    ordinal,inputPath,outputPath,score,sample
                             1,1.in,1.out,100,true
tests/problem-1/1.in         样例输入原样
tests/problem-1/1.out        样例输出原样
```

* **样例输入输出整块作为一个测试点**，不做拆分；
* 该测试点带 `sample=true` 标记，前端会当作公开样例展示；
* 导入契约要求测试点分值之和正好为 100，只放这一个测试点时它就是 **100 分**；
* 题目没有样例（源站 `pre#input` / `pre#output` 为空）时不生成测试点，仍需人工补数据；
* 用 `--no-sample-test` 可以退回"只写题面、不含测试点"的旧行为。

转换器**只从样例取数据**，不会生成额外测试点或凭空造期望输出——那需要可信标程，
否则产出的数据会让正确程序判 WA。需要更多测试点请走项目的 AI 测试点生成流程，
导出包的 `tests/` 结构（`cases.csv` + `N.in` / `N.out`）与它是兼容的。

> **一个提示，不影响结构**：源站会把多组样例拼在同一个 `<pre>` 里。题目 1002 的样例输入是
> `2 100` / `2 22` 两行、输出是 `20` / `6` 两行，实际是两组独立用例。整块作为单个测试点时，
> 只处理单组的程序可能判 WA。转换器按「输入输出行数相同且都大于 1」识别这种写法，
> 在命令结尾点名提醒由人工在管理页面决定是否拆分：
>
> ```
> 注意：以下题目的样例块里可能拼了多组用例，整块作为单个测试点时，只处理单组的程序可能判 WA：
