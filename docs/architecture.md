...
题目内容不可变。题面、资源限制、测试数据或标程变化都创建新版本；比赛和个人计时套卷锁定具体版本。测试点不携带分值，提交得分统一按「通过点数 / 总点数」折算。未满分提交进入错题本，满分后标记已解决，但历史提交、收藏和旧记录保留。
...
状态从 `ANALYZING` 进入 `GENERATING_TESTS`、`GENERATING_SOLUTIONS`、`DIFFERENTIAL_TESTING`，最后到 `VALIDATING`；任何一步可进入人工处理、失败或取消。Agent 每生成一批源码就立即提交一次只编译的沙箱任务（编译门禁），编译失败按产物归属回到对应的生成节点重做，同一阶段在整个运行内最多编译三次，用尽后交管理员处理。测试生成器、两份标程和暴力解互不依赖，因此差分阶段把失败归到哪一批就只重做哪一批，已通过门禁的产物在后续修复轮次中直接复用。全量差分任务仍会重新编译全部源码，编译门禁只用于尽早暴露错误，不作为判分依据。AI 数据始终标记为“练习数据，非官方原始数据”。
