...
async function start(): Promise<void> {
  ...
  if (!autoPublish.value && sampleCount.value > 0) {
    ElMessage.warning("关闭自动发布时请将公开样例数量设为 0，生成后可在编辑页手动勾选");
    return;
  }
  loading.value = true;
  try {
...
<template>
  <section class="content-page admin-page">
    ...
    <section class="admin-tool-surface admin-ai-tool">
      ...
      <el-form-item label="发布方式"><el-checkbox v-model="autoPublish">差分通过后自动发布</el-checkbox></el-form-item>
      <el-form-item v-if="autoPublish" label="公开样例数量"><el-input-number v-model="sampleCount" :min="0" :max="testCaseCount" /></el-form-item>
