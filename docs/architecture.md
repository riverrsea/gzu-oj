...
状态从 `ANALYZING` 进入 `GENERATING_TESTS`、`GENERATING_SOLUTIONS`、`DIFFERENTIAL_TESTING`，最后到 `VALIDATING`；任何一步可进入人工处理、失败或取消。Agent 每生成一批源码就立即提交一次只编译的沙箱任务（编译门禁），编译失败按产物归属回到对应的生成节点重做，同一阶段在整个运行内最多编译三次，用尽后交管理员处理。全量差分任务仍会重新编译全部源码，编译门禁只用于尽早暴露错误，不作为判分依据。AI 数据始终标记为“练习数据，非官方原始数据”。
