...
转换后的 ZIP 包含 `problems.csv`、`statements/`，并在有公开样例时写入 `tests/`：题面的公开样例会被提取成一个测试点（样例输入输出整块写入 `1.in` / `1.out`，`sample=true`），让题目导入后立刻可判。导入契约要求测试点分值之和正好为 100，所以只放这一个测试点时它就是 100 分；不需要样例测试点时加 `--no-sample-test`。转换器只从样例取数据，不生成额外测试点也不凭空造期望输出——更多测试点请走 AI 测试点生成流程。注意源站会把多组样例拼在同一个 `<pre>` 里（例如 1002 的输入 `2 100` / `2 22` 对应输出 `20` / `6` 其实是两组用例），转换器会按「输入输出行数相同且都大于 1」在命令结尾提醒判题口径风险，是否拆分由人工在管理页面决定。导入后题目为草稿，其余测试点在管理员编辑页面中继续录入。
