...
export const api = {
  ...
  startAiRun: (problemVersionId: string, testCaseCount: number, autoPublish = false, sampleCount = 0) =>
    request<AiRun>("/api/v1/admin/test-generation-runs", {
      method: "POST",
      body: JSON.stringify({ problemVersionId, testCaseCount, autoPublish, sampleCount }),
    }),
  /** 查询单次 AI 录题流程的最新状态。 */
  aiRun: (runId: string) => request<AiRun>("/api/v1/admin/test-generation-runs/" + runId),
  ...
  resumeAiRun: (runId: string, body: { action: "reanalyze"; correction?: unknown }) =>
